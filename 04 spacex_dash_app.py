...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df=spacex_df[spacex_df['Launch Site']=='CCAFS LC-40']['class'].value_counts().rename_axis('class').to_frame('counts').reset_index()
        fig = px.pie(filtered_df, values='counts', 
        names='class', 
        title='Total Success Launches by Site CCAFS LC-40')
        return fig
    elif entered_site == 'site2':
        filtered_df=spacex_df[spacex_df['Launch Site']=='VAFB SLC-4E']['class'].value_counts().rename_axis('class').to_frame('counts').reset_index()
        fig = px.pie(filtered_df, values='counts', 
        names='class', 
        title='Total Success Launches by Site VAFB SLC-4E')
        return fig
    elif entered_site == 'site3':
        filtered_df=spacex_df[spacex_df['Launch Site']=='KSC LC-39A']['class'].value_counts().rename_axis('class').to_frame('counts').reset_index()
        fig = px.pie(filtered_df, values='counts', 
        names='class', 
        title='Total Success Launches by Site KSC LC-39A')
        return fig
    elif entered_site == 'site4':
        filtered_df=spacex_df[spacex_df['Launch Site']=='CCAFS SLC-40']['class'].value_counts().rename_axis('class').to_frame('counts').reset_index()
        fig = px.pie(filtered_df, values='counts', 
        names='class', 
